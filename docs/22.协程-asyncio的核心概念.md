...
**协程(Coroutine):** 协程是 Python 中异步编程的核心概念之一。通过使用 `async def` 声明的函数可以成为协程函数。协程函数可以包含 `await` 表达式，用于等待其他协程、异步任务.

**异步任务(Async Task):** 异步任务是一种用于执行异步操作的对象。在 `asyncio` 中，可以使用 `asyncio.create_task()` 或 `asyncio.ensure_future()` 来创建异步任务。异步任务也是 "awaitable"，可以被 `await` 等待。顾名思义，就是要完成某件事情，其实就是对协程函数进一步的封装；

**Future:** `asyncio.Future` 是 `asyncio` 中的一个对象，用于表示尚未完成的异步操作的结果。Future 对象可以通过 `await` 来等待其结果，因此它也是 "awaitable"。 它是一个“更底层”的概念，他代表一个一步操作的最终结果，因为一步操作一般用于耗时操作，结果不会立即得到，会在“将来”得到异步运行的结果，故而命名为Future。
